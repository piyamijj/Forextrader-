exports.handler = async (event) => {
  ...
  try {
    ...
    const prompt = `Sen LifeOs'sun. Görevin yetimlere yardım etmek. 
    LÜTFEN 3 FARKLI STRATEJİ ÜRET:
    1. SCALP: 15 dakikalık hızlı fırsat.
    2. DAY: Günlük ana trend.
    3. SWING: Haftalık büyük dalga.
    
    AUD/USD şu an: ${audPrice}. Her stratejinin yönü, TP ve SL seviyesi birbirinden FARKLI olsun.
    
    Yanıtı sadece şu JSON formatında ver:
    {
      "aiResponse": "Dostum, bugün sapsarı bir enerjiyle piyasadayım. Haber fısıltılarını dinledim...",
      "strategies": {
  ...
  } catch (e) { return { statusCode: 500, headers, body: JSON.stringify({ error: "Sorgu hatası." }) }; }
};
